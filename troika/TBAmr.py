...
class TBAmr(object):
    ...
    def __init__(self, args):
        # from logger import logger
        # get date and timeself.logger = logging.getLogger(__name__)
    ...
    def construct_command(self):
        ...
        cmd = f"snakemake -s {self.resources / 'utils' / 'tbamr.smk'} -j {self.jobs} -d {self.workdir} {singularity} 2>&1 | tee -a {self.workdir / 'tbrnr.log'}"
        return cmd
    ...
    def run_snakemake(self):
        ...
        self.logger.info(f"Now running AMR detection using the TBrnr pipeline with command {cmd}. Please be patient this may take some time.")
